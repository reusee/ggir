...
func CairoSurfaceCreateFromPixbuf(pixbuf *C.GdkPixbuf, scale int, for_window IsWindow) (return__ *C.cairo_surface_t) {
	return__ = C.gdk_cairo_surface_create_from_pixbuf(pixbuf, C.int(scale), for_window.GetWindowPointer())
	return
}
...
and “\#ffffffffffff”).
*/
func ColorParse(spec string) (color C.GdkColor, return__ bool) {
	__cgo__spec := (*C.gchar)(unsafe.Pointer(C.CString(spec)))
	var __cgo__return__ C.gboolean
	__cgo__return__ = C.gdk_color_parse(__cgo__spec, &color)
	C.free(unsafe.Pointer(__cgo__spec))
	return__ = __cgo__return__ == C.gboolean(1)
	return
}
...
func SelectionOwnerSet(owner IsWindow, selection C.GdkAtom, time_ uint32, send_event bool) (return__ bool) {
	...
	var __cgo__return__ C.gboolean
	__cgo__return__ = C.gdk_selection_owner_set(owner.GetWindowPointer(), selection, C.guint32(time_), __cgo__send_event)
	return__ = __cgo__return__ == C.gboolean(1)
	return
}
...
func SelectionOwnerSetForDisplay(display IsDisplay, owner IsWindow, selection C.GdkAtom, time_ uint32, send_event bool) (return__ bool) {
	...
	var __cgo__return__ C.gboolean
	__cgo__return__ = C.gdk_selection_owner_set_for_display(display.GetDisplayPointer(), owner.GetWindowPointer(), selection, C.guint32(time_), __cgo__send_event)
	return__ = __cgo__return__ == C.gboolean(1)
	return
}
